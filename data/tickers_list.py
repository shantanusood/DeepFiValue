...
def tickers(type):
    ...
    if "spy" in type:
        ...
        filename = ""
        if "_" in type:
            filename = './data/tickers/' + type[0:int(type.index('_'))] + '.txt'
        else:
            filename = './data/tickers/' + type+'.txt'
...
def ticker_details(type):
    ...
    list = []
    if "_" in type:
        filename = './data/tickers/' + type[0:int(type.index('_'))] + '.csv'
        start = int(type[int(type.index("_"))+1: int(type.index(":"))])
        end = int(type[int(type.index(":"))+1:])
